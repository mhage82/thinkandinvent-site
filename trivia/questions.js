// Each question: { text, choices: [...], correctIndex }
window.TRIVIA_QUESTIONS = [
    {
      text: "Which ancient seafaring people from the Lebanese coast pioneered long-distance navigation and purple dye?",
      choices: ["Hittites", "Phoenicians", "Assyrians", "Minoans"],
      correctIndex: 1
    },
    {
      text: "Baalbek’s Roman complex is famous for engineering with some of the world’s heaviest stones. What are the 'Trilithon' blocks?",
      choices: [
        "Three cedar wood beams",
        "Three foundation stones ~800–1000 tons each",
        "Three bronze arches",
        "Three granite columns"
      ],
      correctIndex: 1
    },
    {
      text: "The cedar on Lebanon’s flag symbolizes:",
      choices: ["Desert endurance", "Maritime trade", "Holiness and longevity", "Oil wealth"],
      correctIndex: 2
    },
    {
      text: "Which Lebanese-born scientist and former JPL Director is renowned for spaceborne radar and planetary missions?",
      choices: ["Charles Elachi", "Edward Said", "Mustafa Agha", "Marwan Charbel"],
      correctIndex: 0
    },
    {
      text: "Hassan Kamel Al-Sabah (1905–1935) worked on early innovations in:",
      choices: ["Nuclear reactors", "Solar energy and power electronics", "Laser optics", "Cryptography"],
      correctIndex: 1
    },
    {
      text: "Tony Fadell, of Lebanese descent, is best known for leading the creation of:",
      choices: ["Android OS", "The iPod and Nest Thermostat", "Dropbox", "3D printing"],
      correctIndex: 1
    },
    {
      text: "Jeita Grotto near Beirut is primarily celebrated for:",
      choices: [
        "An underground oil field",
        "A natural limestone cave system with rivers and chambers",
        "A WWII bunker network",
        "A man-made metro tunnel"
      ],
      correctIndex: 1
    },
    {
      text: "Lebanon’s highest peak is:",
      choices: ["Mount Hermon (Jabal ash-Shaykh)", "Qurnat as Sawda’ (~3,088 m)", "Sannine (~2,628 m)", "Makmel Ridge (~2,500 m)"],
      correctIndex: 1
    },
    {
      text: "Which university founded in 1866 has a long engineering legacy in Lebanon?",
      choices: ["Lebanese University", "American University of Beirut (AUB)", "Balamand University", "Beirut Arab University"],
      correctIndex: 1
    },
    {
      text: "The Litani is Lebanon’s:",
      choices: ["Largest lake", "Longest river", "Deepest cave", "Tallest dam"],
      correctIndex: 1
    },
    {
      text: "Traditional Phoenician purple dye (Tyrian purple) came from:",
      choices: ["Cedar bark", "Basalt rock", "Murex sea snails", "Beetroot"],
      correctIndex: 2
    },
    {
      text: "A hallmark of Phoenician engineering was:",
      choices: ["Steel skyscrapers", "Advanced shipbuilding and coastal harbors", "Hot-air balloons", "Suspension bridges"],
      correctIndex: 1
    },
    {
      text: "Which Lebanese coastal city was a key Phoenician port and glass-making center?",
      choices: ["Zahle", "Byblos (Jbeil)", "Baalbek", "Bcharre"],
      correctIndex: 1
    },
    {
      text: "The modern project nicknamed the “Beirut River Solar Snake” is primarily:",
      choices: ["A hydro dam", "A series of solar photovoltaic arrays along the river", "A tidal generator", "A pumped-storage plant"],
      correctIndex: 1
    },
    {
      text: "For ties, your event rules state the winner is the one with:",
      choices: ["Fewest wrong answers", "Highest score then fastest time", "Earliest start", "Most skipped questions"],
      correctIndex: 1
    }
  ];
  